...
| その他の折りたたみ端末 / Android 14 | 動作未確認 |

インストールには Android 14 以上が必要です。機種差や未確認の動作は [対応状況（英語）](docs/COMPATIBILITY.en.md)をご覧ください。スクリーンリーダーとの同時使用には対応していません。
...
**Codex、Claude Code などのエージェントに頼む場合：** [エージェント向けインストールガイド（英語）](docs/AGENT_INSTALL.en.md)を参照してください。リポジトリの入口は [AGENTS.md（英語）](AGENTS.md)です。例えば、次のように依頼できます。

> このリポジトリの AGENTS.md とエージェント向けインストールガイドを読み、スマートフォンに FoldPatch をインストールしてください。実行できる作業は進め、端末で私の操作が必要なときに教えてください。既存のアプリデータと設定は保持してください。
