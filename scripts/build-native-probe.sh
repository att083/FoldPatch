...
javac -encoding UTF-8 -source 8 -target 8 -cp "$REACHPAD_ANDROID" -d "$REACHPAD_OUT/classes" "$REACHPAD_ROOT"/experiments/native-screen/*.java
jar cf "$REACHPAD_OUT/classes.jar" -C "$REACHPAD_OUT/classes" .
"$REACHPAD_TOOLS/d8" --min-api 30 --lib "$REACHPAD_ANDROID" --output "$REACHPAD_OUT/dex" "$REACHPAD_OUT/classes.jar"
