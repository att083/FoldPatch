...
javac -encoding UTF-8 -cp "$REACHPAD_ANDROID" -d "$REACHPAD_ROOT/build/tests" \
  "$REACHPAD_ROOT"/src/dev/reachpad/{HangulComposer,PointerGeometry,TouchpadScroll,RangePreview,TouchSide,TrackpadGesture,ProcessShutdown,SetupFlow,ReflowPolicy}.java "$REACHPAD_ROOT"/tests/*.java
for test in HangulComposer PointerGeometry TouchpadScroll RangePreview TouchSide TrackpadGesture ProcessShutdown SetupFlow ReflowPolicy; do
    java -cp "$REACHPAD_ROOT/build/tests:$REACHPAD_ANDROID" "dev.reachpad.${test}Test"
done
