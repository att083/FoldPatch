...
public final class InputProbeActivity extends Activity {
    @Override public void onCreate(Bundle state){super.onCreate(state);LinearLayout row=new LinearLayout(this);row.setBackgroundColor(0xff263542);
        for(int i=0;i<2;i++){final String side=i==0?"left":"right";LinearLayout col=new LinearLayout(this);col.setOrientation(1);col.setPadding(24,100,24,24);
            ...
            input.addTextChangedListener(new TextWatcher(){public void beforeTextChanged(CharSequence s,int a,int c,int f){}public void onTextChanged(CharSequence s,int start,int before,int count){value.setText(s);android.util.Log.i("FoldPatchInput",side+"="+s);}public void afterTextChanged(Editable e){}});
            input.setOnEditorActionListener((v,action,event)->{android.util.Log.i("FoldPatchInput",side+" action="+action);return false;});
