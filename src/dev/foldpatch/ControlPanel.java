...
final class ControlPanel {
    ...
    void setup(){
        ...
        item(c,Settings.canDrawOverlays(a)?"복귀 버튼 권한: 사용 중":"왼쪽 복귀 버튼 사용 허용",()->a.startActivity(new Intent(Settings.ACTION_MANAGE_OVERLAY_PERMISSION,Uri.parse("package:dev.foldpatch"))));
    ...
    private void apps(){
        ...
        Runnable fill=()->{list.removeAllViews();String q=search.getText().toString().toLowerCase(java.util.Locale.ROOT);
            for(ResolveInfo app:apps){if(app.activityInfo.packageName.equals("dev.foldpatch"))continue;String name=app.loadLabel(a.getPackageManager()).toString();if(!name.toLowerCase(java.util.Locale.ROOT).contains(q))continue;
