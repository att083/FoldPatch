...
public final class ShellBridge extends Binder {
    ...
    private static final String OWN_IME="dev.foldpatch/.ReachKeyboard";
    ...
    private ShellBridge(Context context) throws Exception {
        ...
        appUid = context.getPackageManager().getApplicationInfo("dev.foldpatch", 0).uid;
        ...
        nativeScreen=new NativeScreen(context,context.getPackageManager().getApplicationInfo("dev.foldpatch",0).sourceDir);
        main.post(new Runnable(){public void run(){
            boolean was=nativeScreen.active();
            try{if(!nativeScreen.check()&&was)restoreIme();}
            catch(Exception e){
                android.util.Log.w("FoldPatchNative","Retry screen cleanup",e);
    ...
    @Override protected boolean onTransact(int code, Parcel data, Parcel reply, int flags) throws RemoteException {
        ...
        data.enforceInterface("dev.foldpatch.control");
            ...
            if (code == 1) {
                ...
                try {
                    ...
                    if(virtualDisplay!=null){virtualDisplay.resize(width,height,density);virtualDisplay.setSurface(surface);}
                        ...
                        VirtualDisplayConfig config = new VirtualDisplayConfig.Builder("FoldPatch", width, height, density)
                        ...
                        virtualDisplay = displays.createVirtualDisplay("FoldPatch", width, height, density, surface, vdFlags);
                    }
                ...
                try {
                    ...
                    if (!accepted) System.err.println("FoldPatch input rejected");
                ...
                String component;
                if ("probe".equals(target)) component = "dev.foldpatch/.ProbeActivity";
                else if ("latency".equals(target)) component = "dev.foldpatch/.LatencyProbeActivity";
    ...
    private boolean normal(ActivityManager.RunningTaskInfo t) {
        ...
        if(p.equals("dev.foldpatch")||p.equals("com.android.systemui")||p.equals("com.sec.android.app.launcher"))return false;
    ...
    private Bundle command(String action,Bundle b) throws Exception {
        ...
        if(action.equals("enable_pointer_accessibility")){
            String component="dev.foldpatch/dev.foldpatch.PointerAccessibility";
            ...
            for(ActivityManager.RunningTaskInfo t:taskList()){
                ...
                if(t.topActivity!=null&&t.topActivity.getPackageName().equals("dev.foldpatch"))continue;
            ...
            if(!info.exported&&!name.getPackageName().equals("dev.foldpatch")){
            ...
            if(context.getPackageManager().queryIntentActivities(check,0).isEmpty()&&!name.getPackageName().equals("dev.foldpatch"))throw new SecurityException("No launcher entry");
            int task=-1;
            ...
            for(ActivityManager.RunningTaskInfo t:taskList())if(t.topActivity!=null&&t.topActivity.getClassName().equals("dev.foldpatch.MainActivity")){invoke("setFocusedTask",new Class[]{int.class},t.taskId);break;}
        }else if(action.equals("start")){
            process("/system/bin/am","start","--display","0","-n","dev.foldpatch/.MainActivity");
            ...
            if(enabled){
                ...
                if(!current.equals(OWN_IME)&&!current.equals("dev.foldpatch/dev.foldpatch.ReachKeyboard"))previousIme=current;
    ...
    private void restoreIme() throws Exception {
        if(!previousIme.isEmpty()&&!previousIme.equals("null")){
            String current=process("/system/bin/settings","get","secure","default_input_method");
            if(current.equals(OWN_IME)||current.equals("dev.foldpatch/dev.foldpatch.ReachKeyboard"))process("/system/bin/ime","set",previousIme);
        }
    }
    ...
    public static void main(String[] args) throws Exception {
        ...
        Object holder = am.getClass().getMethod("getContentProviderExternal",String.class,int.class,IBinder.class,String.class)
            .invoke(am,"dev.foldpatch.bridge",0,token,"FoldPatch");
        ...
        Bundle result=(Bundle)provider.getClass().getMethod("call",AttributionSource.class,String.class,String.class,String.class,Bundle.class)
            .invoke(provider,shell.getAttributionSource(),"dev.foldpatch.bridge","register",null,extras);
