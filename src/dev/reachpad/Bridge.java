...
final class Bridge {
    ...
    static Parcel data(){Parcel p=Parcel.obtain();p.writeInterfaceToken("dev.reachpad.control");return p;}
    ...
    static void async(String action,Bundle args,Consumer<Bundle> success,Consumer<Exception> failure){
        worker.execute(()->{try{Bundle result=command(action,args);if(success!=null)ui.post(()->success.accept(result));}
            catch(Exception e){android.util.Log.e("ReachPad","control "+action,e);ui.post(()->{if(failure!=null){failure.accept(e);return;}MainActivity a=MainActivity.current;if(a!=null)a.report("작업을 완료하지 못했어요. 설정에서 연결 상태를 확인해 주세요.");});}});
    }
