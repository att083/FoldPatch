...
final class SystemUiFit {
    ...
    void addFrameBackgroundTail(SurfaceControl root,List<SurfaceControl> layers,int width,int height,int usable)throws Exception {
        ...
        try(SurfaceControl.Transaction t=new SurfaceControl.Transaction()) {
            for(int i=0;i<2;i++) {
                ...
                SurfaceControl.Builder b=new SurfaceControl.Builder().setName("ReachPad frame shade corner "+i).setParent(root);call(b,"setContainerLayer",new Class<?>[0]);SurfaceControl viewport=b.build();layers.add(viewport);
    ...
    void stop() {
        ...
        try(SurfaceControl.Transaction t=new SurfaceControl.Transaction()) {
            ...
            t.apply();
        }catch(Exception e){android.util.Log.w("ReachPadNative","Restore system UI",e);}
        try{organizer.close();}
