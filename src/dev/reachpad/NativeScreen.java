...
final class NativeScreen {
    ...
    synchronized Bundle start(float left,float right) throws Exception {
        ...
        try {
            root=layer("ReachPad native screen","setContainerLayer",null);generation=SystemClock.elapsedRealtimeNanos();
            ...
            if(needsReflow()){
                ...
                SurfaceControl black=background=layer("ReachPad damaged gap","setColorLayer",root);
            ...
            pointer=layer("ReachPad cursor","setContainerLayer",root);
            try(SurfaceControl.Transaction t=new SurfaceControl.Transaction()){
                ...
                for(int i=0;i<4;i++){
                    SurfaceControl line=layer("ReachPad cursor stroke","setColorLayer",pointer);
    ...
    private SurfaceControl mirror() throws Exception {
        ...
        SurfaceControl viewport=layer("ReachPad mirror viewport","setContainerLayer",null);
        try(SurfaceControl.Transaction t=new SurfaceControl.Transaction()){
    ...
    synchronized void stop(){
        cleanupPending=true;
        if(root!=null)try(SurfaceControl.Transaction t=new SurfaceControl.Transaction()){call(t,"remove",new Class<?>[]{SurfaceControl.class},root);t.apply();}catch(Exception e){android.util.Log.w("ReachPadNative","Remove",e);}
