...
final class FrameMirror {
    ...
    SurfaceControl start(Context context,Object wm,SystemUiFit systemUi,int width,int height,int usable)throws Exception {
        try {
            SurfaceControl.Builder builder=new SurfaceControl.Builder().setName("ReachPad frame buffer");
            ...
            queue=q.getConstructor(String.class,boolean.class).newInstance("ReachPad frames",true);
            ...
            VirtualDisplayConfig config=new VirtualDisplayConfig.Builder("ReachPad frame output",width,height,metrics.densityDpi)
            ...
            SurfaceControl.Builder rb=new SurfaceControl.Builder().setName("ReachPad output root");call(rb,"setContainerLayer",new Class<?>[0]);outputRoot=rb.build();
