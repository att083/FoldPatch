...
public final class MainActivity extends Activity {
    ...
    private void connectDisplay() {
        ...
        try {
            ...
            android.util.Log.i("ReachPadPerf", "config fast="+lowLatency+" benchmark="+benchmark+" requestedHz="+targetRefreshRate);
        ...
        } catch (Exception e) { updateStatus("연결 오류: " + e.getMessage()); android.util.Log.e("ReachPad", "create display", e); }
    }
    private Parcel data() { Parcel p = Parcel.obtain(); p.writeInterfaceToken("dev.reachpad.control"); return p; }
    private void call(int code, Parcel data) {
        ...
        } catch (Exception e) { updateStatus(e.getMessage()); android.util.Log.e("ReachPad", "command " + code, e); }
        finally { data.recycle(); reply.recycle(); }
    }
    ...
    private void sendMotion(MotionEvent event) {
        ...
        if (lowLatency) {
            ...
            } catch(Exception e) { updateStatus(e.getMessage()); android.util.Log.e("ReachPad","input",e); }
            finally { p.recycle(); }
        } else call(2,p);
        if(benchmark) android.util.Log.i("ReachPadPerf","bridge input="+event.getEventTime()+" callUs="+(System.nanoTime()-start)/1000);
    }
    ...
    private final class SplitView extends GLSurfaceView implements GLSurfaceView.Renderer {
        ...
        @Override public void onSurfaceCreated(GL10 gl,EGLConfig config) {
            ...
            if(lowLatency){
                frameCallbacks=new HandlerThread("ReachPadFrames",android.os.Process.THREAD_PRIORITY_DISPLAY);frameCallbacks.start();
        ...
        @Override public void onDrawFrame(GL10 gl) {
            ...
            if(benchmark) {
                ...
                if(encoded!=lastMarker&&encoded!=0&&age<2000){
                    android.util.Log.i("ReachPadPerf","frame marker="+encoded+" ageMs="+age);lastMarker=encoded;
                }
            }
        }
        ...
        private boolean handleTouch(MotionEvent e) {
            ...
            if(benchmark){
                ...
                android.util.Log.i("ReachPadPerf","host input="+source+" ageMs="+(((SystemClock.uptimeMillis()&0xffffff)-source)&0xffffff));
            }
